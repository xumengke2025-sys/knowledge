...
## v0.7 SecEmp arXiv 学术主语料层

2026-09-22 接入 `secemp9/arxiv-complete` 作为历史全量 arXiv 主语料底座：使用 metadata 构造 8 行业候选池，对高价值论文按 `paper_id` 读取 `paper_text` 做方法、实验、指标、局限与失败模式复核。仓库不复制约 16TB 原始语料；SecEmp 是一次性快照，OpenAlex/Crossref Radar 继续负责增量论文发现。详见 `sources/secemp-arxiv/README.md`、`ACADEMIC-GOVERNANCE.md` 与 `QUALITY-V0.7.md`。
...
> 当前版本：**v0.7.0（SecEmp arXiv 学术主语料层）**  
> 截止日期：2026-09-22  
> 本次深化：**全量学术候选发现 + 按需全文复核 + 专家知识晋级**。
