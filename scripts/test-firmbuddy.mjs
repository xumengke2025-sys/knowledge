...
try{
 ...
 const results=[];for(const [industry,query] of Object.entries(knowledgeQueries)){
  ...
  const importedAcademic=first.equipments[academic.logical_key];assert(importedAcademic,industry+' academic equipment imported');
 ...
 output={status:'PASS',test_type:'actual_FirmBuddy_service_isolated_database',documents:bundle.documents.length,equipments:bundle.equipments.length,experts:expertIndustries.length,academic_equipments:bundle.equipments.filter(e=>e.logical_key.endsWith(':academic')).length,pluginChecks,knowledgeChecks:results,idempotency:'passed',document_equipment_upgrade:'passed',historical_chunks:'retained',cas_conflict:'rejected',live_llm_answers:'not_run',production_import:'not_run'};
