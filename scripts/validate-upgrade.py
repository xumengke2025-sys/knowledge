...
bundle=load(R/'firmbuddy/import-bundle.json');keys={d['logical_key'] for d in bundle['documents']};assert len(keys)==len(bundle['documents']);assert len(bundle['documents'])>=36;ekeys={e['logical_key'] for e in bundle['equipments']};assert len(ekeys)==len(bundle['equipments']);assert len(bundle['equipments'])>=36
